 ...
 * @brief 更新脚本
 * 
 * 该脚本用于定期从配置的 XML 源下载节目数据，并将其存入 MySQL 数据库中。
 * 
 * 作者: Tak
...
function generateXmlFromEpgData($db, $include_future_only) {
    ...
    $xml = new SimpleXMLElement('<?xml version="1.0" encoding="UTF-8"?><tv info-name="by mxdabc" info-url="https://github.com/mxdabc/epgphp"></tv>');
...
function insertDataToDatabase($channelsData, $db) {
    foreach ($channelsData as $channelId => $channelData) {
        ...
        foreach ($channelData['diyp_data'] as $date => $diypProgrammes) {
            ...
            $stmt = $db->prepare("INSERT INTO `epg_data` (`date`, `channel`, `epg_diyp`) VALUES (:date, :channel, :epg_diyp)
ON DUPLICATE KEY UPDATE `epg_diyp` = VALUES(`epg_diyp`)");
